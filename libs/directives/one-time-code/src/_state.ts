...
export const NGTW_ONE_TIME_CODE_STATE = new InjectionToken<
  WritableSignal<NgtwOneTimeCodeState>
>('ngtwOneTimeCodeState');

export const consumeOneTimeCodeState = consumeState(NGTW_ONE_TIME_CODE_STATE);

export function createOneTimeCodeState<Type>(
  newState: CreationState<Type>,
): NgtwOneTimeCodeState {
  const internalState = inject(NGTW_ONE_TIME_CODE_STATE, { optional: true });
  if (!internalState)
    throw new Error(`${NGTW_ONE_TIME_CODE_STATE.toString()} was not provided`);
  ...
  const nextState: NgtwOneTimeCodeState = {
    ...
    isComplete: computed(
      () => internalState().value().length === internalState().digits().length,
    ),
...
export function provideOneTimeCodeState(
  initialState?: Partial<NgtwOneTimeCodeState>,
): Provider[] {
  return [
    {
